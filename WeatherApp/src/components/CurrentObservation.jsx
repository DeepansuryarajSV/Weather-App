...
export default function CurrentObservation({handleFetchWeather}) {

    const [forecastData, setForecastData] = useState(JSON.parse(localStorage.getItem('data')) || {});

    const handleClick = async () => {

        let city = document.getElementById("cityinput").value;
        const response = await handleFetchWeather(city);
    ...
    return (
        <div className='Observation-container m-3 '>
                <div className="input-group mb-3 search-container">
                    <input type="text" className="form-control city-input" id='cityinput' placeholder="Search for cities" aria-label="Recipient's username" aria-describedby="button-addon2"/>
                    <button className="btn btn-outline-primary" type="button" id="button-addon2" onClick={handleClick}>Fetch</button>
                </div>
                <div className="currentObservation d-flex justify-content-between">
                    ...
                    <div>
                    {((forecastData?.currentConditions?.conditions?.toLowerCase().includes('cloud')) || (forecastData?.currentConditions?.conditions?.toLowerCase().includes('overcast')))  && (
